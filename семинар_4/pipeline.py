...
Команды:
    python pipeline.py ingest            # стратегия B: recursive (умный чанкинг)
    python pipeline.py ingest --naive    # стратегия A: fixed-size 2000 символов
    python pipeline.py ask "Кто жаловался на push-уведомления?"

Примечание: ingest и eval НЕ требуют LLM (работают на эмбеддере + BM25).
LLM-клиент создаётся лениво — только когда реально вызывается команда ask.
...
# LLM-клиент создаём лениво: ingest/eval не требуют ни LLM_BASE_URL, ни ключа.
_client = None
MODEL = get_model()


def get_client():
    """Ленивая инициализация LLM-клиента — только при вызове ask()."""
    global _client
    if _client is None:
        _client = make_client()
    return _client
...
def ingest(strategy: str = "recursive"):
    ...
    for f in sorted(DATA_DIR.glob("*.txt")):
        ...
        # Стратегия A — fixed-size 2000; Стратегия B — recursive
        if strategy == "naive":
...
def ask(query: str):
    ...
    resp: RAGAnswer = get_client().chat.completions.create(
        model=MODEL,
...
if __name__ == "__main__":
    ...
    cmd = sys.argv[1]
    if cmd == "ingest":
        strategy = "naive" if "--naive" in sys.argv else "recursive"
