...
Реализовано:
- в промпт передаём ТОЛЬКО answer + used_tools, НЕ raw_trace (иначе Критик
  «подсматривает» рассуждения Исполнителя и соглашается с ними);
- temperature вынесена в параметр (по умолчанию 0.7 — чтобы Критик не работал
  зеркалом Планировщика). Домашка С6 (часть 3) переопределяет её на 0.0/0.7
  для замера угодливости.
"""
...
def critic(
    ...
    *,
    ...
    # Собираем answers_text ТОЛЬКО из answer + used_tools (без raw_trace).
