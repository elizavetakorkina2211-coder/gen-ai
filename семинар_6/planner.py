...
ПРАВИЛА:
1. ЛЮБАЯ арифметика (деление, умножение, среднее, рост «во сколько раз»,
   накопленный итог, проценты) выполняется ТОЛЬКО через calculate.
   Никогда не считай в уме — это ошибка класса C. Если подвопрос требует
   вычисления, у него в expected_tools обязан быть calculate.
...
4. Если задача не решается имеющимися tools — верни reasoning с объяснением
   и subquestions=[]. НЕ выдумывай get_cumulative_inflation, get_cagr,
   get_average_fx_rate и подобные несуществующие инструменты.
...
def planner(question: str, *, feedback: str | None = None) -> Plan:
    ...
    Args:
        ...
        feedback: комментарий Критика или Валидатора после неудачной попытки;
                  при наличии — передаётся как дополнительное пользовательское
                  сообщение, чтобы Планировщик учёл замечание при перепланировке.
    """
    client = make_client()
    messages: list[dict] = [
