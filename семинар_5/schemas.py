...
TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Сравнить ОДНУ макрометрику в двух периодах: вернуть значения, "
                "их разницу (delta = b - a) и отношение (ratio = b / a). "
                "Используй для вопросов «во сколько раз выросло», «на сколько "
                "изменилось», «сравни X тогда и сейчас». Не вызывай get_fx_rate/"
                "get_key_rate по отдельности и не считай delta/ratio руками — "
                "этот инструмент делает оба замера и арифметику сам."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "metric": {
                        ...
                        "description": (
                            "Что сравниваем: key_rate — ключевая ставка; "
                            "fx_USD/fx_EUR/fx_CNY — курс валюты к рублю; "
                            "cpi — инфляция % г/г; unemployment — безработица %."
                        ),
                    },
                    "period_a": {
                        "type": "string",
