...
TRACE_JSONL = Path(__file__).resolve().parent / "trace.jsonl"


def _log_step(run_id: str, record: dict, path: Path) -> None:
    """Дописать одну JSONL-строку события в trace.jsonl (open в режиме 'a').

    record уже содержит step и либо (call, args, obs), либо (final).
    Сюда добавляем общий run_id и метку времени."""
    line = {"run_id": run_id, "ts": datetime.datetime.now().isoformat(timespec="seconds")}
    line.update(record)
...
- compare_periods: сравнить одну метрику в двух периодах (delta и ratio сразу)
...
def run_agent(
    ...
    *,
    ...
    model = get_model()
    run_id = uuid.uuid4().hex  # ДЗ-2: один id на все строки этого прогона
    tpath = trace_path or TRACE_JSONL
    ...
    for step in range(1, max_iter + 1):
        ...
        if not msg.tool_calls:
            ...
            _log_step(run_id, {"step": step, "final": msg.content}, tpath)
            return _finish(
                {
                    "answer": msg.content,
                    "structured": None,
        ...
        if submit is not None:
            ...
            _log_step(run_id, {"step": step, "final": ans.answer}, tpath)
            return _finish(
                {
                    "answer": ans.answer,
                    "structured": ans,
    ...
    _log_step(
        run_id,
        {"step": max_iter, "final": None, "error": f"исчерпан лимит шагов max_iter={max_iter}"},
        tpath,
    )
    return _finish(
        {
...
def main():
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Путь к JSONL-логу шагов (по умолчанию trace.jsonl рядом с agent.py)",
    )
    a = ap.parse_args()
