...
# 5. Сравнение метрики в двух периодах (ДЗ к семинару 5)
...
def _split_period(period: str) -> tuple[int, int, _date]:
    """
    Разобрать "YYYY-MM" или "YYYY-MM-DD" в (год, месяц, дату).

    Для point-метрик нужна конкретная дата: из голого "YYYY-MM" берём
    ПОСЛЕДНИЙ календарный день месяца (семантика «на конец месяца», как у
    Росстата), но не позже сегодняшнего дня.
    """
    parts = period.strip().split("-")
    if len(parts) == 3:
        d = datetime.strptime(period.strip(), "%Y-%m-%d").date()
        return d.year, d.month, d
    if len(parts) == 2:
        year, month = int(parts[0]), int(parts[1])
        if not (1 <= month <= 12):
            raise ValueError(f"месяц вне диапазона 1..12: {period!r}")
...
def _value_for(metric: str, period: str) -> dict:
    """Вернуть {'date': ..., 'value': ..., 'source': ...} либо {'error': ...}."""
    kind = _COMPARE_METRICS[metric]
    year, month, d = _split_period(period)

    if metric == "key_rate":
        r = get_key_rate(on_date=d.isoformat())
        if "error" in r:
            return r
        return {"date": r.get("date", d.isoformat()), "value": r["rate"], "source": r["source"]}
    ...
    if metric == "cpi":
        r = get_inflation(year, month)
        if "error" in r:
            return r
        return {"date": f"{year:04d}-{month:02d}", "value": r["cpi_yoy"], "source": r["source"]}

    if metric == "unemployment":
        r = get_unemployment(year, month)
        if "error" in r:
            return r
        return {"date": f"{year:04d}-{month:02d}", "value": r["unemployment"], "source": r["source"]}

    # сюда не дойдём — metric уже проверен в compare_periods
    return {"error": f"метрика {metric} не поддержана"}


def compare_periods(metric: str, period_a: str, period_b: str) -> dict:
    """
    Сравнить значение метрики в двух периодах. Под капотом — существующие
    get_fx_rate / get_key_rate / get_inflation / get_unemployment.
    ...
    Returns:
        {"metric": ...,
         ...
         "source": "..."}
        либо {"error": "..."} — если метрика не та или один из периодов недоступен.
    """
    if metric not in _COMPARE_METRICS:
        return {
            "error": f"неизвестная метрика {metric!r}. Доступны: "
            + ", ".join(_COMPARE_METRICS)
        }

    try:
        a = _value_for(metric, period_a)
        b = _value_for(metric, period_b)
    except ValueError as e:
        return {"error": f"плохой период: {e}"}

    if "error" in a:
        return {"error": f"период A ({period_a}) недоступен: {a['error']}"}
    if "error" in b:
        return {"error": f"период B ({period_b}) недоступен: {b['error']}"}
    ...
    return {
        "metric": metric,
        "a": {"date": a["date"], "value": va},
        "b": {"date": b["date"], "value": vb},
        "delta": round(vb - va, 6),
        "ratio": round(vb / va, 6) if va else None,
        "source": source,
    }
